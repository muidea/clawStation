...
CLAWCONSOLE_WINDOWS_GNU_BIN := $(CLAWCONSOLE_DIR)/src-tauri/target/x86_64-pc-windows-gnu/debug/clawstation.exe
CLAWCONSOLE_WINDOWS_MSVC_BIN := $(CLAWCONSOLE_DIR)/src-tauri/target/x86_64-pc-windows-msvc/debug/clawstation.exe
CLAWCONSOLE_WINDOWS_GNU_WEBVIEW2 := $(CLAWCONSOLE_DIR)/src-tauri/target/x86_64-pc-windows-gnu/debug/WebView2Loader.dll
CLAWCONSOLE_WINDOWS_MSVC_WEBVIEW2 := $(CLAWCONSOLE_DIR)/src-tauri/target/x86_64-pc-windows-msvc/debug/WebView2Loader.dll
CLAWOPS_WINDOWS_GNU_BIN := $(CLAWOPS_DIR)/target/x86_64-pc-windows-gnu/debug/clawops.exe
CLAWOPS_WINDOWS_MSVC_BIN := $(CLAWOPS_DIR)/target/x86_64-pc-windows-msvc/debug/clawops.exe
WINDOWS_README := $(ROOT_DIR)/packaging/windows-README.txt
WINDOWS_GNU_ZIP := $(OUTPUT_DIR)/claw-windows-gnu-debug.zip
WINDOWS_MSVC_ZIP := $(OUTPUT_DIR)/claw-windows-msvc-debug.zip
...
help:
	@printf "%s\n" \
		...
		"make build-windows  - 编译 Windows 调试版，并输出到 output/windows-*/" \
		"make package-windows - 编译并打包 Windows 调试 zip (Linux/WSL -> gnu, Windows -> msvc)" \
...
$(CLAWCONSOLE_RELEASE_BIN):
	cd "$(CLAWCONSOLE_DIR)" && cargo tauri build --release --no-bundle
...
$(CLAWCONSOLE_WINDOWS_GNU_BIN):
	cd "$(CLAWCONSOLE_DIR)" && $(MAKE) tauri-build-windows-gnu

$(CLAWCONSOLE_WINDOWS_MSVC_BIN):
	cd "$(CLAWCONSOLE_DIR)" && $(MAKE) tauri-build-windows-msvc

$(CLAWOPS_WINDOWS_GNU_BIN):
	$(MAKE) -C "$(CLAWOPS_DIR)" build-windows-gnu

$(CLAWOPS_WINDOWS_MSVC_BIN):
	$(MAKE) -C "$(CLAWOPS_DIR)" build-windows-msvc
...
stage-windows-gnu:
	cd "$(CLAWCONSOLE_DIR)" && $(MAKE) tauri-build-windows-gnu
	$(MAKE) -C "$(CLAWOPS_DIR)" build-windows-gnu
	rm -rf "$(WINDOWS_GNU_OUT)"
	mkdir -p "$(WINDOWS_GNU_OUT)"
	cp "$(CLAWCONSOLE_WINDOWS_GNU_BIN)" "$(WINDOWS_GNU_OUT)/"
	cp "$(CLAWCONSOLE_WINDOWS_GNU_WEBVIEW2)" "$(WINDOWS_GNU_OUT)/"
	cp "$(CLAWOPS_WINDOWS_GNU_BIN)" "$(WINDOWS_GNU_OUT)/"
	cp "$(WINDOWS_README)" "$(WINDOWS_GNU_OUT)/README.txt"
	@printf "%s\n" \
		"Windows GNU 构建完成：" \
		"  $(WINDOWS_GNU_OUT)/clawstation.exe" \
		"  $(WINDOWS_GNU_OUT)/WebView2Loader.dll" \
...
stage-windows-msvc:
	cd "$(CLAWCONSOLE_DIR)" && $(MAKE) tauri-build-windows-msvc
	$(MAKE) -C "$(CLAWOPS_DIR)" build-windows-msvc
	rm -rf "$(WINDOWS_MSVC_OUT)"
	mkdir -p "$(WINDOWS_MSVC_OUT)"
	cp "$(CLAWCONSOLE_WINDOWS_MSVC_BIN)" "$(WINDOWS_MSVC_OUT)/"
	cp "$(CLAWCONSOLE_WINDOWS_MSVC_WEBVIEW2)" "$(WINDOWS_MSVC_OUT)/"
	cp "$(CLAWOPS_WINDOWS_MSVC_BIN)" "$(WINDOWS_MSVC_OUT)/"
	cp "$(WINDOWS_README)" "$(WINDOWS_MSVC_OUT)/README.txt"
	@printf "%s\n" \
		"Windows MSVC 构建完成：" \
		"  $(WINDOWS_MSVC_OUT)/clawstation.exe" \
		"  $(WINDOWS_MSVC_OUT)/WebView2Loader.dll" \
		"  $(WINDOWS_MSVC_OUT)/clawops.exe" \
		"  $(WINDOWS_MSVC_OUT)/README.txt"

$(WINDOWS_GNU_ZIP): stage-windows-gnu
	rm -f "$(WINDOWS_GNU_ZIP)"
	cd "$(WINDOWS_GNU_OUT)" && zip -rq "$(WINDOWS_GNU_ZIP)" .
	@printf "打包完成：%s\n" "$(WINDOWS_GNU_ZIP)"

$(WINDOWS_MSVC_ZIP): stage-windows-msvc
	rm -f "$(WINDOWS_MSVC_ZIP)"
	cd "$(WINDOWS_MSVC_OUT)" && zip -rq "$(WINDOWS_MSVC_ZIP)" .
	@printf "打包完成：%s\n" "$(WINDOWS_MSVC_ZIP)"
